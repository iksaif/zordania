...
<elseif cond='{msg_act} == "read_env"'>
	<if cond='isset({msg_infos})'>
		...
		<img src="img/mbr_logo/{msg_infos[menv_to]}.png" title="{msg_infos[mold_pseudo]}" class="blason" />
		Envoyé à 
		...
		[ <a href="msg-del_env.html?menv_id={msg_infos[menv_id]}" title="Effacer ce message">
...
<elseif cond='{msg_act} == "del_env" OR {msg_act} == "del_rec"'>
	<if cond='isset({msg_need_conf})'>
		...
		<if cond='is_array({msg_id})'>
			...
			les <math oper="count({msg_id})" /> messages
		</if>
		<elseif cond='{msg_id}!=0'> le message </elseif>
		<else> tous les messages </else>
		<if cond='{msg_act} == "del_env"'> envoyés </if><else> reçus </else> ?
...
<elseif cond='{msg_act} == "new"'>
	...
	<form action="msg-send.html" method="post">
	...
	<p><label for="msg_titre">Titre</label>
	<input type="text" maxlength="150" name="msg_titre" id="msg_titre" value="{msg_titre}" tabindex="2" /></p>
	
	<include file='commun/bbcode.tpl' cache='1' />
	<p><textarea id="message" cols="60" rows="11" name="msg_texte" tabindex="3">{msg_texte}</textarea> </p>
