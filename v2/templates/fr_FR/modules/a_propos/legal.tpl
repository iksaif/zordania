<p class="menu_module">
[<a href="a_propos.html" title="les membres de l'équipe"> L'équipe </a>]
- [<a href="a_propos-rec.html" title="Les récompenses"> Récompenses </a>]
- [<a href="a_propos-le_site.html" title="l'évolution du site"> Le site </a>]
- [<a href="a_propos-boutons.html" title="Les boutons"> Boutons </a>]
- [<a href="a_propos-legal.html" title="Mentions légales"> Mentions légales </a>]
</p>

<h1>Mentions Légales</h1> 	  	 
 
<h3>Contact</h3>

<p>Iksaif
 - <a href="mailto:%77%65%62%6das%74%65r@&#122&#111rd%61%6e&#105a.%63%6f%6d?subject=Zordania">Contact</a></p>

<p>Pifou
 - <a href="mailto:pifou@zordania.com" id="NoSpamEmailHyperlink1">pifou@zordania.com</a>
</p>
 		  		 	  	
<h3>Conditions d'utilisation</h3>

<p>
Tout accès ou utilisation de zordania.com est soumis aux dispositions suivantes. 
Le webmaster de zordania.com se réserve le droit de modifier ces présentes dispositions librement et sans annonce préalable. 
</p>

<p>
Les visiteurs de zordania.com accèdent et utilisent ce site et son contenu sous leur propre responsabilité. 
Le webmaster de zordania.com ne peut être tenu pour responsable de perte ou dommage direct ou indirect de quelque nature que ce soit résultant d'un accès ou d'une utilisation de ce site, d'une erreur ou omission dans ce site, de virus issus de ce site ou de la non accessibilité de ce site.
</p>

<p>
Tout inscrit pourra être exclu par le webmaster de zordania.com si cet inscrit est jugé nuisible à zordania.com par le webmaster de zordania.com
</p>

<h3>Sécurité des comptes</h3>
<p>
Ne pouvant garantir les informations entrées lors de l'inscription, notamment les mots de passe et emails, l'administrateur ne pourra être tenu responsable des problèmes liés à une mauvaise utilisation du jeu, tels que le vol de compte. 
</p>
<p>
Il en va de même pour les utilisations abusives du jeu dont fait partie le pillage d'alliance entre autres. 
</p>
<p>
Par ailleurs pour votre sécurité, vous êtes invité à utiliser des mots de passe complexes mêlant chiffres, lettres et symboles. 
 </p>
 <p>
Il est à noter que l'administrateur peut à tout moment, et sur votre demande, modifier votre mot de passe, si vous rencontriez des problèmes lors de la modification de vos informations personnelles.
</p>

<h3>Espaces de discussion</h3>

<p>
Les forums, les messages privés, les commentaires de news et le chat IRC sont des espaces de discussion destinés à vous permettre de dialoguer avec les membres et visiteurs du site.
Les données qui y figurent ne peuvent être collectées ou utilisées à d'autres fins.
<p/>

<p>
Les discussions sont surveillées par des modérateurs qui sont susceptibles de censurer ou supprimer toute intervention qui ne serait pas en relation avec le thème de discussion abordé, la ligne éditoriale du site, les règles de courtoisie, ou qui serait contraire à la loi.
</p>

<h3>Respect de la vie privée</h3>

<p>
Vous disposez d'un droit d'accès, de modification, de rectification et de suppression des données qui vous concernent (art. 34 de la loi "Informatique et Libertés" du 6 janvier 1978).
</p>
<p>
Vous pouvez accéder à vos informations dans votre Profil en vue de les modifier à votre guise. Si vous rencontrez un problème, vous pouvez également vous adresser au Webmaster en vue de modifier ces données.
</p>
<p>
Les données personnelles collectées lors de votre inscription ne servent que sur le site; elles ne sont en aucun cas cédées à des tiers. De plus, votre mail ne figure pas sur le site.
</p>

<h3>Cookies</h3>

<p>
Afin de pouvoir se connecter au site, un <a href="http://fr.wikipedia.org/wiki/Cookies" title="Plus d'infos">cookie</a> est créé. Vous pouvez le refuser, mais il sera alors impossible d'utiliser le site correctement.
</p>

<h3>Hébergeur</h3>

<p>
Le Serveur de zordania.com est hébergé par la société Dedibox.
</p>

<p>
<pre>
DEDIBOX Société par action simplifiée au capital de 37000 Euros est une société du groupe Iliad, 
immatriculée au Registre du Commerce et des Sociétés de Paris sous le numéro RCS PARIS B 484 961 206, 
numéro de TVA FR30484961206.

Coordonnées :
DEDIBOX SAS
BP 80342
75365 PARIS CEDEX 08
Fax : 01 73 50 26 05
Tel : 01 73 50 31 65 - Attention, pas de support par téléphone !
Email site : http://www.dedibox.fr
Email support technique : https://console.dedibox.fr/accueil/assistance/
Horaires d'ouverture : du Lundi au Vendredi, de 9h-12h 14h30-16h
</pre>
</p>


<h3>Licences</h3>
<p>
Copyright © 2004-2010
</p>
<p>
Le contenu de ce site Web (à savoir, le concept du jeu Zordania, les règles et les textes) est la propriété du gestionnaire de zordania.com. 
Toute reproduction, même partielle, est interdite sans autorisation. <br/>
Les blasons sont la propriété de leur(s) auteur(s) respectif(s).<br/
Les Smileys sont proposés par Deepnight.net.
<br/>
Les scripts (http://www.zordania.com/js/ et http://www.zordania.com/templates/) peuvent être librement réutilisés.
</p>
