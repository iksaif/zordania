...
<?php
	...
	$result = $db->query('SELECT g_id, g_title FROM '.$db->prefix.'groups WHERE g_id!='.$group_id.' ORDER BY g_title') or error('Unable to fetch user group list', __FILE__, __LINE__, $db->error());
...
while ($cur_group = $db->fetch_assoc($result))
	echo "\t\t\t\t\t\t\t\t".'<tr><th scope="row"><a href="admin_groups.php?edit_group='.$cur_group['g_id'].'">Edit</a> - <a href="admin_groups.php?del_group='.$cur_group['g_id'].'">Remove</a></th><td>'.pun_htmlspecialchars($cur_group['g_title']).'</td></tr>'."\n";
