...
if (!$pun_user['is_guest'])
	$result = $db->query('SELECT t.subject, t.closed, t.num_replies, t.sticky, f.id AS forum_id, f.forum_name, fp.post_replies FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$id.' AND t.moved_to IS NULL') or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
else
	$result = $db->query('SELECT t.subject, t.closed, t.num_replies, t.sticky, f.id AS forum_id, f.forum_name, fp.post_replies, 0 FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$id.' AND t.moved_to IS NULL') or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
...
$is_admmod = (is_admin() || is_modo()) ? true : false;
...
$result = $db->query('SELECT u.alliance, u.alliance_id, u.race, u.points, u.signature, u.num_posts,  p.id, p.poster AS username, p.poster_id, p.poster_ip, p.poster_email, p.message, p.hide_smilies, p.posted, p.edited, p.edited_by, g.g_id, g.g_user_title, o.user_id AS is_online FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'users AS u ON u.id=p.poster_id INNER JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id LEFT JOIN '.$db->prefix.'online AS o ON (o.user_id=u.id AND o.user_id!=1 AND o.idle=0) WHERE p.topic_id='.$id.' ORDER BY p.id LIMIT '.$start_from.','.$pun_user['disp_posts'], true) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
while ($cur_post = $db->fetch_assoc($result))
{
	...
	{
		$username = '<a href="'.SITE_URL.'member-view.html?mid='.$cur_post['poster_id'].'">'.pun_htmlspecialchars($cur_post['username']).'</a>';
		$user_title = "<img src=\"".SITE_URL."img/groupes/{$cur_post['g_id']}.png\" alt=\"{$cur_post['g_user_title']}\" /> ". get_title($cur_post);

		// Format the online indicator
		$is_online = ($cur_post['is_online'] == $cur_post['poster_id']) ? '<strong>'.$lang_topic['Online'].'</strong>' : $lang_topic['Offline'];

		$user_avatar = "<img src=\"".MBR_LOGO_URL.$cur_post['poster_id'].".png\" alt=\"{$cur_post['username']}\" />";

		// We only show location, register date, post count and the contact links if "Show user info" is enabled
		if ($pun_config['o_show_user_info'] == '1')
		{
			if ($pun_config['o_show_post_count'] == '1' || is_admin() || is_modo())
				$user_info[] = '<dd>'.$lang_common['Posts'].': '.$cur_post['num_posts'];
		}

		if($cur_post['alliance_id']) {
			$user_info[] = "<dd><img src=\"".ALL_LOGO_URL.$cur_post['alliance_id']."-thumb.png\" alt=\"{$cur_post['alliance']}\" /><a href=\"".SITE_URL."alliances-view.html?al_aid={$cur_post['alliance_id']}\">{$cur_post['alliance']}</a>";
		}
		
		$user_info[] = "<dd>Points: {$cur_post['points']}";
		$user_info[] = "<dd>Race: <img src=\"".SITE_URL."img/{$cur_post['race']}/{$cur_post['race']}.png\" alt=\"\" />";
