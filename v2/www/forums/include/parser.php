...
$smiley_text = array_keys($_smileys);
$smiley_img = array_values($_smileys);
...
/*	global $smiley_text, $smiley_img;
	...
	for ($i = 0; $i < $num_smilies; ++$i)
		$text = preg_replace("#(?<=.\W|\W.|^\W)".preg_quote($smiley_text[$i], '#')."(?=.\W|\W.|\W$)#m", '$1<img src="'.SITE_URL.'img/smile/'.$smiley_img[$i].'" alt="'.substr($smiley_img[$i], 0, strrpos($smiley_img[$i], '.')).'" />$2', $text);

	return substr($text, 1, -1);*/ return $text;
}
