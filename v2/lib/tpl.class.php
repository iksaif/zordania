...
class Template
{
	function Template() //constructeur
	{
		...
		$this->set_tmp_dir('../tmp/'); // dossier du cache 
		...
		$this->replaceby[0] = '<img src="img/$4/$2/$6.png" alt="{$2[$4][alt][$6]}" title="{$2[$4][alt][$6]}" />';
		$this->replace[1] = '#<(bbimg)(res|unt|trn|btc|src|comp)\s*(?:(?:\s*(?:(?:race=(\\\\\'|")(.*?)\\3)|(?:type=(\\\\\'|")(.*?)\\5)))+)\s*/>#s';
	...
	function var_back($var) //callback pour compiler le template
	{
		...
		elseif($var[1] == 'zimg')
		{
			if($var[2] == 'bar' or $var[2] == 'ba2' or $var[2] == 'ba3')
			{
				...
				if ($var[2] == 'ba2') $size = 'moyennes';
				else if ($var[2] == 'ba3') $size = 'grandes';
				else $size = 'petites';
				$width1 = "<math oper=\"floor({$var[4]} / {$var[6]}*100)\" />";
				$width2 = "<math oper=\"floor(({$var[6]}-{$var[4]}) / {$var[6]}*100)\" />";
				return "<div class=\"barres_$size\">
