...
9. full - returns this array: $browser_name, $version_number, $ie_version, $dom_browser, 
	$safe_browser, $os, $os_number, $s_browser [the last is the browser search string from the browser array]
*******************************************/
	...
	*/
	if ( !$b_repeat )
	{
		...
		*/
		...
		$a_browser_types[] = array( 'netpositive', false, 'netp' );// beos browser
		$a_browser_types[] = array( 'lynx', false, 'lynx' );
		$a_browser_types[] = array( 'webtv', false, 'webtv' );
		$a_browser_types[] = array( 'amaya', false, 'amaya' );
		$a_browser_types[] = array( 'ibrowse', false, 'ibrowse' );// amiga browser
		$a_browser_types[] = array( 'icab', false, 'icab' );// mac browser
		$a_browser_types[] = array( 'omniweb', false, 'omni' );// mac osx browser
		
		// search engine spider bots:
		$a_browser_types[] = array( 'googlebot', false, 'google' );// google
		$a_browser_types[] = array( 'yahoo-verticalcrawler', false, 'yahoo' );// old yahoo bot
		$a_browser_types[] = array( 'yahoo! slurp', false, 'yahoo' ); // new yahoo bot
		$a_browser_types[] = array( 'fast-webcrawler', false, 'fast' );// Fast AllTheWeb
		$a_browser_types[] = array( 'msnbot', false, 'msn' );// msn search 
		$a_browser_types[] = array( 'ask jeeves', false, 'ask' ); //jeeves/teoma
		$a_browser_types[] = array( 'teoma', false, 'ask' );//jeeves teoma
		$a_browser_types[] = array( 'scooter', false, 'scooter' );// altavista 
		$a_browser_types[] = array( 'zyborg', false, 'looksmart' );// looksmart 
		$a_browser_types[] = array( 'almaden', false, 'ibm' );// ibm almaden web crawler 
		$a_browser_types[] = array( 'baiduspider', false, 'baidu' );// Baiduspider asian search spider
		$a_browser_types[] = array( 'psbot', false, 'psbot' );// psbot image crawler 

		// netscape 4 and earlier tests, put last so spiders don't get caught
		$a_browser_types[] = array( 'mozilla/4.', false, 'ns' );
		$a_browser_types[] = array( 'mozilla/3.', false, 'ns' );
		$a_browser_types[] = array( 'mozilla/2.', false, 'ns' );
...
function which_os ( $browser_string, $browser_name, $version_number  )
{
	...
	*/
	...
	$a_os = array( 'beos', 'os2', 'amiga', 'mac', 'nt', 'win', $a_unix, $a_linux );
