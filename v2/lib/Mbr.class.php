...
class Mbr {
	...
	// constructeur private pour interdire de faire new Mbr();
	...
	/* Ajoute un membre */
	static function add($login, $pass, $mail, $lang, $etat, $gid, $decal, $ip, $design, $parrain)
	{
		...
		$sql = "INSERT INTO ".self::$__sql->prebdd."mbr (mbr_login, mbr_pseudo, mbr_pass, mbr_mail, mbr_lang, mbr_etat, mbr_gid, mbr_decal, mbr_ldate, mbr_inscr_date, mbr_lip, mbr_design, mbr_parrain)";
		$sql.= " VALUES ('$login','$login','$pass','$mail','$lang','$etat','$gid','$decal',NOW(),NOW(),'$ip',$design, $parrain)";

		self::$__sql->query($sql);

		if(self::$__sql->errno) /* Une des clefs unique existe déjà ! */
			return 0;
		else {
			$mid =  self::$__sql->insert_id();
	...
	static function edit($mid, $new)
	{
		return edit_gen(array('mid' => $mid), $new);
	}

	/* modifie en général */
	static function edit_gen($cond, $new) {
			...
			$sql = "UPDATE ".self::$__sql->prebdd."mbr SET ";
			if(isset($new['pseudo'])) {	
			...
			if(isset($new['vlg'])) {
				$sql1 = "UPDATE ".self::$__sql->prebdd."leg SET leg_name='$vlg'";
				$sql1 .= " WHERE leg_mid = $mid AND leg_etat IN(".LEG_ETAT_VLG.",".LEG_ETAT_BTC.")";
				self::$__sql->query($sql1);
			}

			self::$__sql->query($sql);
			return self::$__sql->affected_rows();// + edit_frm($mid, $new);
	}
	
	/* Permet de détecter les multis comptes */
	static function get_infos_ip($ip = '',$gid = 0)
	{
		...
		if(!$ip){
			$sql="SELECT COUNT(*) as ip_nb,mbr_lip FROM ".self::$__sql->prebdd."mbr GROUP BY mbr_lip HAVING ip_nb > 1";
			$temp_array = self::$__sql->make_array($sql);
	...
	static function get_pos($mid) {
		...
		$sql .=" FROM ".self::$__sql->prebdd."mbr ";
		$sql .=" JOIN ".self::$__sql->prebdd."map ON mbr_mapcid = map_cid";
		$sql .=" WHERE mbr_mid = $mid";
	...
	static function ini($mid, $pseudo, $vlg, $race, $cid, $gid, $sexe=1) {
	...
	static function reini($mid, $pseudo, $vlg, $race, $cid, $oldcid, $gid, $sexe=1) {
		cls_map($mid, $oldcid);
		cls_aly($mid);
		cls_unt($mid);
		cls_hro($mid);
		...
		self::ini($mid, $pseudo, $vlg, $race, $cid, $gid, $sexe);
	}

	/* Libère les trucs d'un compte */
	static function cls($mid, $cid, $race) {
		add_old_mbr($mid) ;
		cls_aly($mid);
		cls_unt($mid);
		cls_btc($mid);
	...
	static function del($mid) {
	
		$mid = protect($mid, "uint");
	
		$sql = "DELETE FROM ".self::$__sql->prebdd."mbr WHERE mbr_mid = $mid ";
	
		$res = self::$__sql->query($sql);
		return self::$__sql->affected_rows();
	}

	static function get_nb_race($race = 0)
	{

		$race = protect($race, "uint");
	
		$sql="SELECT mbr_race,COUNT(*) as race_nb FROM ".self::$__sql->prebdd."mbr ";
	...
	static function can_atq_lite($mbr_array, $points, $mid, $groupe, $alaid, $dpl_array = false)
	{
		$mid = protect($mid, "uint");
	...
	static function upload_logo($mid, $fichier)
	{
		$mid = protect($mid, "uint");
	...
	static function get_lite($mid) {
		$mid = protect($mid, "uint");
		if(!$mid)
			return array();
		else
			return self::get_gen(array('mid' => $mid));
	}

	/* Sur les pages d'infos */
	static function get_full($mid) {
		$mid = protect($mid, "uint");
		if(!$mid)
			return array();
		else
			return self::get_gen(array('mid' => $mid, 'full' => true));
	}

	/* membres d'une alliance */
	static function get_aly($aid) {
		$aid = protect($aid, 'uint');
		$cond = array('op' => 'AND', 'list' => true, 'limite1' => ALL_MAX, 'aid' => $aid);
		$cond['orderby'] = array('DESC', 'points');
		if(!$aid)
			return false;
		else
			return self::get_gen($cond);
	}

	/* En général : */
	static function get_gen($cond) {
		...
		if($aid && (!$full && !$list))
			$sql.= "JOIN ".self::$__sql->prebdd."al_mbr ON mbr_mid = ambr_mid ";

		if($full || $list || count($dst) == 2) {
			$sql.= "LEFT JOIN ".self::$__sql->prebdd."map ON mbr_mapcid = map_cid ";
			if(!$aid) {
				$sql.= "LEFT JOIN ".self::$__sql->prebdd."al_mbr ON mbr_mid = ambr_mid ";
				$sql.= "LEFT JOIN ".self::$__sql->prebdd."al ON ambr_aid = al_aid ";
			} else {
				$sql.= "JOIN ".self::$__sql->prebdd."al_mbr ON mbr_mid = ambr_mid ";
				$sql.= "JOIN ".self::$__sql->prebdd."al ON ambr_aid = al_aid ";
			}
		} else {
			$sql.= "LEFT JOIN ".self::$__sql->prebdd."al_mbr ON mbr_mid = ambr_mid ";
		}
	...
	static function get_id_vlg($mid, $etat){ // récupére l'id du village suivant l'état 

		$mid = protect($mid, "uint");
		$etat = protect($etat, "uint");
	
		$sql = "SELECT leg_id FROM ".self::$__sql->prebdd."leg WHERE leg_mid = $mid AND leg_etat = $etat";
		$res = self::$__sql->query($sql);
		$lid = self::$__sql->result($res, 0, 'leg_id');
	
		return $lid;
	}

	/* Compte le nombre de membres */
	static function count($cond = array()) {
		$cond['count'] = true;
		$ret = self::get_gen($cond);
	...
	static function exists($mid) {
		$mid = protect($mid, "uint");
		if($mid)
			return self::count(array('mid' => $mid));
		else
			return 0;
	}

	/* Pour les listes de membres */
	static function get_liste($cond, $limite1, $limite2, $orderby) {
	...
	static function get_race_info($races = array()) {
		global $_races;
	...
	static function get_nb($cond = array()) {
	...
	static function add_old_mbr($mid) {

		$mid = protect($mid, "uint");
	
		$sql = "INSERT INTO ".self::$__sql->prebdd."mbr_old (mold_mid, mold_pseudo, mold_mail, mold_lip) ";
		$sql.= "SELECT mbr_mid, mbr_pseudo, mbr_mail, mbr_lip FROM ".self::$__sql->prebdd."mbr WHERE mbr_mid = $mid";
		return self::$__sql->query($sql);
	}

	static function get_old($cond = array()) {
		...
		$sql = "SELECT * FROM ".self::$__sql->prebdd."mbr_old ";
		if($mid || $pseudo || $ip) {
			$sql .= "WHERE ";
	...
	static function get_log_ip($mid = 0, $ip = '', $select = '',$gid = ''){
		...
		if($select == 'full')
			...
			FROM '.self::$__sql->prebdd.'mbr_log
			LEFT JOIN '.self::$__sql->prebdd.'mbr ON mlog_mid = mbr_mid
			LEFT JOIN '.self::$__sql->prebdd.'mbr_old ON mlog_mid = mold_mid';
		else
		...
		if($select == 'full' and $gid != GRP_DIEU)
			$sql.= " AND mbr_gid NOT IN (".GRP_GARDE.",".GRP_PRETRE.",".GRP_DEMI_DIEU.",".GRP_DIEU.",".GRP_DEV.",".GRP_ADM_DEV.") ";
		$sql .= ' ORDER BY '.self::$__sql->prebdd.'mbr_log.mlog_date DESC LIMIT 0, 15';

		return self::$__sql->make_array($sql);
	}

	//ajoute une surveillance
	static function add_surv($mid, $mid_admin, $type, $cause){
		$mid = protect($mid, "uint");
		$type = protect($type, "uint");
		$mid_admin = protect($mid_admin, "uint");
		$cause = protect($cause, "string");
	
		$sql = "INSERT INTO ".self::$__sql->prebdd."surv (surv_id, surv_mid, surv_admin, surv_debut, surv_etat, surv_type, surv_cause) ";
	...
	static function get_surv($mid){
		...
		$sql.= " FROM ".self::$__sql->prebdd."surv";
		$sql.= " WHERE surv_mid=".$mid;
		$sql.= " AND surv_etat = ".SURV_OK;
		return self::$__sql->make_array($sql);
	}

	static function get_surv_by_sid($sid){
	...
	static function get_surv_list(){
		...
		$sql.= " FROM ".self::$__sql->prebdd."surv ";
		$sql.= " JOIN ".self::$__sql->prebdd."mbr as mbr ON mbr.mbr_mid = surv_mid";
		$sql.= " JOIN ".self::$__sql->prebdd."mbr as mbr2 ON mbr2.mbr_mid = surv_admin";
		$sql.= " WHERE surv_etat = ".SURV_OK;
		return self::$__sql->make_array($sql);
	}
	static function get_fin_surv_list (){
	...
	static function close_surv($sid){
		$sid =  protect($sid, "uint");

	
		$sql = " UPDATE ".self::$__sql->prebdd."surv ";
		$sql.= " SET surv_fin = NOW(), surv_etat = ".SURV_CLOSE;
		$sql.= " WHERE surv_id = ".$sid;
		return self::$__sql->query($sql);
	}

	static function is_surv($mid){
		$mid = protect($mid, "uint");
		$array = get_surv($mid);
		if(empty($array))
			return false;
	...
	static function move($mid, $move_to) {
		$mid =  protect($mid, "uint");
		$move_to =  protect($move_to, "uint");

		if($mid and $move_to)
			return self::$__sql->query("CALL move_member($mid, $move_to);");
		else
			return false;
	}
