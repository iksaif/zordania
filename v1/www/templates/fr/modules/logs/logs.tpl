...
<a href="index.php?file=admin&amp;module=logs&amp;act=crons_result" title="Crons Temps">Temps crons</a>
]-[
<a href="index.php?file=admin&amp;module=logs&amp;act=crons_cron" title="Crons Resultats">Résultats crons</a>
]
<hr />
</p>
...
<elseif cond='|{log_act}| == "bonus"'>
	...
	<foreach cond='|{bon_array}| as |{rien}| => |{result}|'>
			...
			<h4>Par : {result[mbr_pseudo]} Le 
				...
				Quoi : {result[bon_res_nb]} <img src="img/{result[mbr_race]}/res/{result[bon_res_type]}.png" /><br/>
				Ok : {result[bon_ok]}
			</div>
			<br/>
			</if>
...
<elseif cond='|{log_act}| == "crons_temps"'>
 ...
 <a href="index.php?file=admin&amp;module=logs&amp;act=view&amp;log_file=crons/out/{result[name]}" title="Voir {result[name]}">{result[name]}</a>
...
<elseif cond='|{log_act}| == "crons_result"'>
 <if cond='is_array(|{log_array}|)'>
 <foreach cond='|{log_array}| as |{result}|'>
 <a href="index.php?file=admin&amp;module=logs&amp;act=view&amp;log_file=crons/bench/{result[name]}" title="Voir {result[name]}">{result[name]}</a>
  - {result[size]} octets - {result[date]} <br/>
 </foreach>
 </if>
 <else>
